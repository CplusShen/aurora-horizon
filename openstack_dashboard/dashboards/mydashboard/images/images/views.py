...
from openstack_dashboard.dashboards.mydashboard.images.images \
    import forms as mydashboard_forms
from openstack_dashboard.dashboards.mydashboard.images.images \
    import tables as mydashboard_tables
from openstack_dashboard.dashboards.mydashboard.images.images \
    import tabs as mydashboard_tabs


class CreateView(forms.ModalFormView):
    form_class = mydashboard_forms.CreateImageForm
    form_id = "create_image_form"
    modal_header = _("Create An Image")
    submit_label = _("Create Image")
    submit_url = reverse_lazy('horizon:mydashboard:images:images:create')
    template_name = 'mydashboard/images/images/create.html'
...
class UpdateView(forms.ModalFormView):
    form_class = mydashboard_forms.UpdateImageForm
    form_id = "update_image_form"
    modal_header = _("Edit Image")
    submit_label = _("Edit Image")
    submit_url = "horizon:mydashboard:images:images:update"
    template_name = 'mydashboard/images/images/update.html'
    success_url = reverse_lazy("horizon:mydashboard:images:index")
    ...
    def get_object(self):
        try:
            ...
            url = reverse('horizon:mydashboard:images:index')
...
class DetailView(tabs.TabView):
    tab_group_class = mydashboard_tabs.ImageDetailTabs
    ...
    def get_context_data(self, **kwargs):
        ...
        image = self.get_data()
        table = mydashboard_tables.ImagesTable(self.request)
        context["image"] = image
        context["url"] = self.get_redirect_url()
        context["actions"] = table.render_row_actions(image)
        choices = mydashboard_tables.ImagesTable.STATUS_DISPLAY_CHOICES
        image.status_label = filters.get_display_label(choices, image.status)
        return context

    @staticmethod
    def get_redirect_url():
        return reverse_lazy('horizon:mydashboard:images:index')
